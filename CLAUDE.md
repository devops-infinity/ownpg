...
- `bash -n` on every tracked shell script, then `shellcheck -x -P .` and `shfmt -d` on `tools/*.sh` and `.githooks/pre-commit`/`.githooks/pre-push`: every shell script parses, lints clean, and stays formatted. `bats tools/tests/toolset.bats` covers `tools/lib.sh`, `tools/install-hooks.sh`, both git hooks, and the comment-detection pattern this file's own gate uses.
